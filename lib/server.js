...
server.commonServer = function(req, res){
    ...
    req.on('end', function(){
        ...
        let selectHander = typeof(server.router[trimmedUrl]) !== 'undefined' ? server.router[trimmedUrl] : handlers.notFound; 

        selectHander = trimmedUrl.indexOf('public/') != -1 ? handlers.public : selectHander;
        ...
        selectHander(reqData, function(statusCode, payload, contentType){
            ...
            if (contentType in contentObject){
                ...
                // Use the payload called back by the selected handler or default to empty string
                stringedPayload = typeof(payload) != undefined ? payload : '';
            }

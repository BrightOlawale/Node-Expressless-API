            ...
            checkList.forEach((element) => {
                    Data.read('checks', element, function(err, mainCheckData){
                        if (!err && mainCheckData){
                            ...
                            debug('Error: Error reading one of the check data');
                        }
                    })
            });
        } else{
            debug('Error: No check to process')
        }
    })
}
...
workers.validateCheckData = function(checkData){
    ...
    if (checkData.id && checkData.url && checkData.phone&&
        ...
        checkData.timeoutSeconds
            ...
            debug('Error: Some check value are not well formatted');
        }
}
    ...
    Data.update('checks', updatedCheckData.id, updatedCheckData, function(err){
        if (!err){
            if (alertRequired){
                ...
                debug('No changes to check yet, Check returned with response code: '+outcomeObject.responseCode)
            }
        } else{
            debug('Error updating the check new data');
        }
    })
};
...
workers.sendChangeStateAlert = function(updatedCheckData){
    ...
    helper.twilioSendSms(updatedCheckData.phone, message, function(err){
        if (!err){
            debug('Success: Successfully sent a change in check state alert to user')
        } else{
            debug('Error: Could not send a change in check state sms alert to user')
        }
    })
}
...
workers.log = function(checkData, outcomeObject, state, alertRequired, timeChecked){
    ...
    logFile.append(logFileName, logString, function(err){
        if (!err){
            ...
            debug('FAILED: could not log to file');
        }
    })
};
...
workers.compressLogs = function(){
    ...
    logFile.list(false, function(err, logs){
        if (!err && logs && logs.length > 0){
            ...
            debug("Could not find any log to compress");
        }
    })
}
...
workers.init = function(){
    ...
    console.log('\x1b[33m%s\x1b[0m', 'Background worker now running');
